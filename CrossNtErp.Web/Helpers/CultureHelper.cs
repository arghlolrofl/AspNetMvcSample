...
namespace CrossNtErp.Web.Helpers {
    public static class CultureHelper {

        static CultureHelper() {
            foreach (CultureInfo ci in CultureInfo.GetCultures(CultureTypes.AllCultures).Where(c => c.Name.Length > 0))
                _availableCultures.Add(ci.Name);
        }

        // Valid cultures
        private static readonly List<string> _availableCultures = new List<string>();
        //{
        ...
        public static string GetImplementedCulture(string name) {
            ...
            if (_availableCultures.Where(c => c.Equals(name, StringComparison.InvariantCultureIgnoreCase)).Count() == 0)
                return GetDefaultCulture(); // return Default culture if it is invalid
                                            // if it is implemented, accept it
            if (_supportedCultures.Where(c => c.Equals(name, StringComparison.InvariantCultureIgnoreCase)).Count() > 0)
        ...
        public static string GetDefaultCulture() {
            return _supportedCultures[0]; // return Default culture
        }
